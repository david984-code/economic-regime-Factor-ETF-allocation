...
def _difficult_sharpe(df: pd.DataFrame) -> float:
    ...
    return (
        float(segs.loc[mask, "Strategy_Sharpe"].mean()) if mask.any() else float("nan")
    )
...
def main():
    ...
    print("   tolerance (baseline):         0.000")
    print(f"   tolerance (experiment):       {TAU:.3f}")
    print("   Implementation: trade-vector filter, renorm after partial suppression")
    print("   Parameters changed vs prior experiments: tolerance only")
    if VOL_LOOKBACK != 63:
    ...
    for label, prior, cur in [
        ...
        print(
            f"   {label:20} {p_s:>10} {c_s:>10} {d_s:>10} {'OK' if ok else 'DIFF':>6}"
        )
    print()
    print("   Note: tolerance=0.0 must be identical to unmodified engine output.")
    print(
        "   If delta > tolerance above, new tolerance parameter may be altering engine"
    )
    ...
    print("=" * 72)
    print(
        f"   {'Metric':30} {'Baseline (tau=0)':>16} {'Exp (tau=1.5%)':>14} {'Delta':>10}"
    )
    print("   " + "-" * 72)
    rows = [
    ...
    print(
        f"   Sharpe delta < +0.02?             {'YES' if ks_shr else 'NO ':3}  ({shr_d:+.3f})"
    )
    print(
        f"   CAGR delta < +0.25%?              {'YES' if ks_cagr else 'NO ':3}  ({cagr_d:+.2%})"
    )
    print(
        f"   No difficult-period improvement?  {'YES' if ks_diff else 'NO ':3}  "
    ...
    if has_to:
        ...
        print()
        print("   Attribution context (full-history baseline, prior diagnostic):")
        print("     Vol-rebalancing component: 79.9% of total TO  (~90pp of 113pp)")
        print("     Signal-driven component:   43.5% of total TO  (~49pp of 113pp)")
        print(
            "     tau=1.5% targets small vol-driven moves; large signal moves pass through"
        )
        print()
        if to_d > 0.005:
            print(f"   FLAG: turnover INCREASED (+{to_d:.2%}).")
            print(
                "     Renormalization after partial suppression can redistribute weight"
            )
            print(
                "     to other assets, generating additional trades. Trade-vector filter"
            )
            print(
                "     does not guarantee aggregate turnover reduction when some assets pass."
            )
        elif abs(to_d) < 0.01:
            print(f"   FLAG: turnover nearly unchanged (|delta|={abs(to_d):.2%}).")
            print("     Most vol-rebalancing trades exceed tau=1.5% per asset.")
            print("     tau=1.5% is too coarse to suppress the dominant churn source.")
        elif to_d < 0:
            pct_red = -to_d / to_b
            print(
                f"   Turnover reduced by {-to_d:.2%} ({pct_red:.1%} relative reduction)."
            )
    ...
    if has_to:
        ...
        if to_d <= 0:
            ...
            print(
                "   Expected: risk-off sleeve (IEF, GLD, TLT) accounts for most suppression"
            )
            print(
                "   based on attribution finding that risk-off drives dominant vol churn."
            )
        else:
            print(
            ...
            print(
                "   The renormalization step spread suppressed weight to passing assets,"
            )
            print("   creating larger-than-expected trades in other positions.")
    ...
    for year, label in [(2020, "2020 COVID"), (2022, "2022 rate shock")]:
        ...
        if np.isnan(b_shr):
            ...
            print(
                f"     (min_train_months=60 from {FAST_START}: first OOS starts ~2023)"
            )
            print("     Crash-period check deferred to full walk-forward validation.")
        else:
            d_shr = e_shr - b_shr
            d_mdd = e_mdd - b_mdd
            print(f"     Segments covering {year}: available")
            print(
                f"     Sharpe: baseline={_f(b_shr)}  exp={_f(e_shr)}  delta={_f(d_shr, sign=True)}"
            )
            print(
                f"     MaxDD:  baseline={_pct(b_mdd)}  exp={_pct(e_mdd)}  delta={_pct(d_mdd, sign=True)}"
            )
            if e_mdd < b_mdd - 0.01:
                print(
                    f"     FLAG: MaxDD worsens by >1% in {year} -> reject regardless of Sharpe"
                )
            elif e_shr > b_shr:
    ...
    print(
        f"   Sharpe delta >= +0.02?            {'YES' if esc_shr else 'NO '}  ({shr_d:+.3f})"
    )
    print(
        f"   CAGR delta >= +0.25%?             {'YES' if esc_cagr else 'NO '}  ({cagr_d:+.2%})"
    )
    print(
        f"   MaxDD delta > +1.0%?              {'YES' if esc_mdd else 'NO '}  ({mdd_d:+.2%})"
    )
    print(f"   Difficult-period improvement?     {'YES' if esc_diff else 'NO '}")
    print()
    if to_increased:
        print(
            f"   SPECIAL: turnover INCREASED ({to_d:+.2%}) -- renorm effect. Reject unless"
        )
        print("   Sharpe improves materially enough to justify the extra cost.")
        print()
    ...
    if not escalate or kill:
        ...
        if to_increased:
            print(
                "   - TO increased: renorm after partial suppression redistributes weight."
            )
            print(
                "     The trade-vector filter does not bound aggregate turnover when some"
            )
            print("     assets still trade and renorm inflates their target sizes.")
            print(
                "     Options: (a) try proportional allocation approach -- keep suppressed"
            )
            print(
                "     assets at prior weight, do NOT renorm until an asset crosses tau;"
            )
            print("     (b) try much lower tau (0.5%) to raise suppression rate;")
            print(
                "     (c) accept that target-to-target TO is structural given 63d vol scaling."
            )
        elif has_to and abs(to_d) < 0.01:
            print(
                "   - TO unchanged: vol-driven trades are predominantly >1.5% per asset."
            )
            print(
                "     tau=1.5% is too coarse. Try tau=0.5% or accept turnover as structural."
            )
        elif has_to and to_d < -0.01 and shr_d < -0.02:
            print(
                "   - TO reduced but Sharpe worsened: the small vol-driven adjustments are"
            )
            print(
                "     load-bearing for performance -- not pure noise. The 63d vol scaling"
            )
            print("     provides useful risk adjustment that should not be suppressed.")
            print(
                "     Next: accept turnover level, or explore alternative signal structure."
            )
        elif has_to and to_d < -0.01:
            print(
                "   - TO reduced, Sharpe roughly neutral (near-zero but below kill threshold)."
            )
            print(
                "     Fast-mode window (2018-2024) does not include 2020/2022 OOS coverage."
            )
            print(
                "     This result is ambiguous -- escalate to full walk-forward to validate."
            )
