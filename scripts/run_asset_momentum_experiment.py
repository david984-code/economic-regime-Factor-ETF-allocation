...
def main():
    ...
    report_lines.append("- Maximum diversification within risk-on sleeve")
    ...
    for _, row in results_df.iterrows():
        ...
        report_lines.append(
            f"- Sharpe: {row['sharpe']:.3f} ({row['sharpe'] - baseline['sharpe']:+.3f} vs baseline)"
        )
        report_lines.append(
            f"- Turnover: {row['turnover']:.1%} ({row['turnover'] - baseline['turnover']:+.1%} vs baseline)"
        )
        report_lines.append("")
    ...
    for _, row in results_df.iterrows():
        report_lines.append(
            f"| {row['method_name']:>30} | {row['robustness_score']:>5.3f} |"
        )
    ...
    if best_method["method"] == "equal_weight":
        ...
        avg_filtered_sharpe = results_df[results_df["method"] != "equal_weight"][
            "sharpe"
        ].mean()
        avg_filtered_turnover = results_df[results_df["method"] != "equal_weight"][
            "turnover"
        ].mean()

        if avg_filtered_sharpe < baseline_sharpe:
            report_lines.append(
                f"- Asset filtering reduces Sharpe on average (avg: {avg_filtered_sharpe:.3f} vs baseline: {baseline_sharpe:.3f})"
            )
        if avg_filtered_turnover > baseline["turnover"]:
            report_lines.append(
                f"- Asset filtering increases turnover (avg: {avg_filtered_turnover:.1%} vs baseline: {baseline['turnover']:.1%})"
            )

        report_lines.append(
            "- Maximum diversification (7 assets) is better than concentration"
        )
        report_lines.append(
            "- SPY momentum already captures market-level trends effectively"
        )
        report_lines.append(
            "- Individual asset momentum adds noise without signal improvement"
        )
        report_lines.append("")
    ...
    if best_method["method"] != "equal_weight":
        report_lines.append(
            "1. **Fine-tune filtering thresholds** - test different momentum cutoffs"
        )
        report_lines.append(
            "2. **Test dynamic N** - vary number of assets based on signal strength"
        )
    else:
        report_lines.append(
            "1. **Expand asset universe** - test sector ETFs, international, alternatives"
        )
        report_lines.append(
            "2. **Test regime-conditional universes** - vary risk-on sleeve by macro regime"
        )
        report_lines.append(
            "3. **Test concentration limits** - cap max position (e.g., 10-20% per asset)"
        )

    report_lines.append(
        "4. **Test alternative signal construction** - cross-sectional momentum, relative strength"
    )
