...
def _check_lookahead_bias() -> list[str]:
    ...
    issues.append(
        "[OK] Regime timing: Month-end regime values are forward-filled to next month (CORRECT)"
    )
    ...
    # 2. Rebalance timing
    issues.append(
        "[OK] Rebalance timing: Triggered on first day of new month (CORRECT)"
    )
    ...
    issues.append(
        "  - Z-score normalization uses mean/std from full history (lookahead)"
    )
    issues.append(
        "  - FIX REQUIRED: Momentum should be computed incrementally or lagged"
    )
...
def _check_return_calculation() -> list[str]:
    ...
    checks.append(
        "[OK] Transaction costs: Applied on rebalance dates only (line 254-260)"
    )
    checks.append(
        f"  - Cost = turnover * COST_BPS (currently {COST_BPS * 10000:.1f} bps)"
    )
...
def _check_slippage_assumptions() -> list[str]:
    ...
    assumptions.append(
        "- [OK] Monthly rebalancing: Realistic for institutional strategies"
    )
    assumptions.append(
        "- [OK] 8 bps cost: Reasonable for liquid ETFs with moderate size"
    )
    assumptions.append(
        "- [OPTIMISTIC] Close-to-close: Real execution has intraday drift"
    )
    assumptions.append(
        "- [OPTIMISTIC] No partial fills: Assumes full position changes execute"
    )
...
def main():
    ...
    print()
    print(
        f"At current {COST_BPS * 10000:.1f} bps: Annual cost ~ {base_turnover * COST_BPS:.2%}"
    )
    print()
