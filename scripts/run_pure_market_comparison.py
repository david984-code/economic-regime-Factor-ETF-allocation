...
def main():
    ...
    hybrid_vs_pure_cagr = (
        hybrid_overall["Strategy_CAGR"] - pure_overall["Strategy_CAGR"]
    )
    hybrid_vs_pure_sharpe = (
        hybrid_overall["Strategy_Sharpe"] - pure_overall["Strategy_Sharpe"]
    )
    hybrid_vs_pure_ic = ic_hybrid["ic_avg"] - ic_pure["ic_avg"]

    if hybrid_vs_pure_cagr > 0.002 and hybrid_vs_pure_sharpe > 0.01:
        report_lines.append(
            "**YES**: The 20% macro overlay improves both CAGR and Sharpe vs pure market."
        )
    elif hybrid_vs_pure_cagr < -0.002 or hybrid_vs_pure_sharpe < -0.01:
        report_lines.append("**NO**: Pure market model outperforms hybrid.")
    else:
        report_lines.append(
            "**NEUTRAL**: Macro overlay has minimal impact on performance."
        )

    report_lines.append("")
    report_lines.append(
        f"- Hybrid vs Pure: CAGR {hybrid_vs_pure_cagr:+.2%}, Sharpe {hybrid_vs_pure_sharpe:+.3f}, IC {hybrid_vs_pure_ic:+.4f}"
    )
    report_lines.append("")
    ...
    models = [
        (
            "Baseline",
            baseline_overall["Strategy_CAGR"],
            baseline_overall["Strategy_Sharpe"],
            baseline_overall.get("Strategy_Turnover", 0.0),
            ic_baseline["ic_avg"],
        ),
        (
            "Hybrid",
            hybrid_overall["Strategy_CAGR"],
            hybrid_overall["Strategy_Sharpe"],
            hybrid_overall.get("Strategy_Turnover", 0.0),
            ic_hybrid["ic_avg"],
        ),
        (
            "Pure Market",
            pure_overall["Strategy_CAGR"],
            pure_overall["Strategy_Sharpe"],
            pure_overall.get("Strategy_Turnover", 0.0),
            ic_pure["ic_avg"],
        ),
    ]
    ...
    report_lines.append(
        f"- Hybrid improves IC by {ic_improvement_hybrid:+.4f} vs baseline"
    )
    ...
    baseline_win_rate = (
        baseline_segs["Strategy_CAGR"] > baseline_segs["SPY_CAGR"]
    ).mean()
    ...
    pure_is_best = (
        pure_overall["Strategy_CAGR"] >= hybrid_overall["Strategy_CAGR"]
        and pure_overall["Strategy_Sharpe"] >= hybrid_overall["Strategy_Sharpe"] - 0.005
    )
