...
def _weight_stability(
    prices: pd.DataFrame, vol_lookback: int, risk_on_me: pd.Series
) -> dict:
    ...
    w_risk_on = {
        a: (1.0 / len(RISK_ON_ASSETS_BASE) if a in RISK_ON_ASSETS_BASE else 0.0)
        for a in ASSETS
    }
    w_risk_off = {
        a: (1.0 / len(RISK_OFF_ASSETS_BASE) if a in RISK_OFF_ASSETS_BASE else 0.0)
        for a in ASSETS
    }
    ...
    for d in rebalance_dates:
        ...
        std_d = {
            a: float(row[a]) if a in row.index and pd.notna(row[a]) else None
            for a in TICKERS
        }
...
def main():
    ...
    print("-" * 44)
    print(
        f"  Config VOL_LOOKBACK before runs: {_cfg.VOL_LOOKBACK}  [must be {VOL_BASE}]"
    )
    print(
        f"  Engine VOL_LOOKBACK before runs: {_eng.VOL_LOOKBACK}  [must be {VOL_BASE}]"
    )
    ...
    print("  signal pipeline: 24M SPY momentum -> expanding z-score -> sigmoid(z*0.25)")
    print("  rebalance: monthly, first trading day of each month, both runs")
    print(
        "  blend: risk_on * inv_vol_scaled_risk_on + (1-risk_on) * inv_vol_scaled_risk_off"
    )
    print("  cost model: |w_target(t) - w_target(t-1)| * COST_BPS, both runs")
    ...
    print("  " + "-" * 68)
    print(
        f"  {'CAGR':30} {_fmt_pct(cagr_b):>14} {_fmt_pct(cagr_e):>12} {_fmt_pct(cagr_d, sign=True):>10}"
    )
    print(
        f"  {'Sharpe':30} {_fmt_f(shr_b):>14} {_fmt_f(shr_e):>12} {_fmt_f(shr_d, sign=True):>10}"
    )
    print(
        f"  {'MaxDD':30} {_fmt_pct(mdd_b):>14} {_fmt_pct(mdd_e):>12} {_fmt_pct(mdd_d, sign=True):>10}"
    )
    print(
        f"  {'Vol':30} {_fmt_pct(vol_b):>14} {_fmt_pct(vol_e):>12} {_fmt_pct(vol_d, sign=True):>10}"
    )
    if has_to:
        print(
            f"  {'Turnover':30} {_fmt_pct(to_b):>14} {_fmt_pct(to_e):>12} {_fmt_pct(to_d, sign=True):>10}"
        )
    else:
    ...
    print(
        f"  Sharpe delta < +0.02?             {'YES' if ks_sharpe else 'NO ':3}  ({shr_d:+.3f})"
    )
    print(
        f"  CAGR delta < +0.25%?              {'YES' if ks_cagr else 'NO ':3}  ({cagr_d:+.2%})"
    )
    ds_b_s = _fmt_f(ds_b) if not np.isnan(ds_b) else "n/a"
    ds_e_s = _fmt_f(ds_e) if not np.isnan(ds_e) else "n/a"
    print(
        f"  No difficult-period improvement?  {'YES' if ks_no_diff else 'NO ':3}  (baseline={ds_b_s}, exp={ds_e_s})"
    )
    print(
    ...
    if has_to:
        ...
        attr_total = 1.1325
        expected_reduction_lo = (
            to_b * (attr_vol_component / attr_total) * 0.25
        )  # 25% of vol component
        expected_reduction_hi = (
            to_b * (attr_vol_component / attr_total) * 0.60
        )  # 60% of vol component
        print("  Attribution context:")
        print(
            "    Vol-rebalancing component (63d baseline, full history): 90.46% of 113.25% total"
        )
        print(
            f"    Expected TO reduction range (25-60% of vol component): "
        ...
        if has_to and not np.isnan(to_d):
            if to_d < -expected_reduction_lo:
                print(
                    f"    Result: reduction of {_fmt_pct(-to_d)} is WITHIN expected range -> vol smoothing effective"
                )
            elif to_d > 0:
                print(
                    "    Result: turnover INCREASED -> unexpected. Smoothing may be disrupting netting."
                )
            else:
                print(
                    f"    Result: reduction of {_fmt_pct(-to_d)} is BELOW expected range -> smoothing insufficient at 126d"
                )
        if to_ratio > 1.5:
            print(
                "    HIGH CHURN FLAG: experiment TO > 1.5x baseline. Escalation threshold = +0.04 Sharpe."
            )
    else:
    ...
    for a in risk_off_check + risk_on_check:
        ...
        s63 = stab_63.get(
            a, {"mean": float("nan"), "std": float("nan"), "range": float("nan")}
        )
        s126 = stab_126.get(
            a, {"mean": float("nan"), "std": float("nan"), "range": float("nan")}
        )
        ...
        print(
            f"  {a:6} {atype:8} {s63['mean']:>10.4f} {s63['std']:>10.4f} {s63['range']:>11.4f} "
            f"{s126['mean']:>11.4f} {s126['std']:>10.4f} {s126['range']:>12.4f} {reduced:>13}"
        )

    n_reduced = sum(
        1
        for a in risk_off_check
    ...
    if n_reduced == len(risk_off_check):
        print(
            "  CONFIRMED: 126d vol smoothing reduces weight churn in all risk-off assets"
        )
    elif n_reduced == 0:
        print(
            "  WARNING: no weight std reduction in risk-off sleeve -- smoothing not effective here"
        )
    else:
        print(
            f"  PARTIAL: {n_reduced}/{len(risk_off_check)} risk-off assets show reduced weight churn"
        )
    ...
    for year, label in [(2020, "2020 COVID"), (2022, "2022 rate shock")]:
        ...
        if len(sb) == 0:
            ...
            print("    (min_train_months=60 from 2018-01-01 -> first OOS ~2023)")
            continue
        ...
        b_to = (
            sb["Strategy_Turnover"].mean()
            if "Strategy_Turnover" in sb
            else float("nan")
        )
        e_to = (
            se["Strategy_Turnover"].mean()
            if "Strategy_Turnover" in se
            else float("nan")
        )
        print(f"    Segments covering {year}: {len(sb)}")
        print(
            f"    Sharpe:   baseline={_fmt_f(b_shr)}  exp={_fmt_f(e_shr)}  delta={_fmt_f(e_shr - b_shr, sign=True)}"
        )
        print(
            f"    MaxDD:    baseline={_fmt_pct(b_mdd)}  exp={_fmt_pct(e_mdd)}  delta={_fmt_pct(e_mdd - b_mdd, sign=True)}"
        )
        if not np.isnan(b_to):
            print(
                f"    Turnover: baseline={_fmt_pct(b_to)}  exp={_fmt_pct(e_to)}  delta={_fmt_pct(e_to - b_to, sign=True)}"
            )
        if e_mdd < b_mdd - 0.01:
            print(
                f"    FLAG: experiment worsens MaxDD by >1% in {year} -> reject signal regardless of overall Sharpe"
            )
        elif e_shr > b_shr:
    ...
    print(
        f"  Sharpe delta >= {shr_thresh:.2f}?              {'YES' if esc_shr else 'NO '}  ({shr_d:+.3f})"
    )
    print(
        f"  CAGR delta >= +0.25%?              {'YES' if esc_cagr else 'NO '}  ({cagr_d:+.2%})"
    )
    print(
        f"  MaxDD improves > +1.0%?            {'YES' if esc_mdd else 'NO '}  ({mdd_d:+.2%})"
    )
    ...
    if to_increased:
        print(
            f"  SPECIAL FLAG: turnover INCREASED (+{to_d:.2%}). Vol smoothing failed to reduce churn."
        )
        print(
            "  This overrides any Sharpe improvement -- the structural hypothesis is rejected."
        )
    ...
    if kill or (not escalate):
        ...
        if has_to and to_d > -0.01:
            ...
            print(
                "    VOL_LOOKBACK=126 is still short enough that monthly vol estimates"
            )
            print(
                "    change substantially. Consider 189d or quarterly vol recalibration."
            )
        elif has_to and to_d < -0.02 and shr_d < 0:
            print("  - Turnover decreased but Sharpe worsened.")
            print(
                "    The inverse-vol scaling with 63d window is providing useful risk management."
            )
            print(
                "    Slower weights hold excess risk-off allocation longer in stress periods,"
            )
            ...
            print(
                "    This is a CONDITIONAL ACCEPT: the reduction is real but the fast-mode"
            )
