...
def main():
    ...
    if momentum_avg_sharpe > meanrev_avg_sharpe:
        ...
        report_lines.append(
            f"**Winner: {winner}** (+{delta_sharpe:.3f} Sharpe, +{delta_cagr:.2%} CAGR vs Mean-Rev)"
        )
    else:
        ...
        report_lines.append(
            f"**Winner: {winner}** (+{delta_sharpe:.3f} Sharpe, +{delta_cagr:.2%} CAGR vs Momentum)"
        )
    ...
    if overall_ic_corr > 0.5:
        report_lines.append(
            f"- **Strong positive correlation** between IC and Sharpe (r={overall_ic_corr:.2f})"
        )
        report_lines.append(
            "- Higher predictive power translates to better realized performance"
        )
    elif overall_ic_corr < -0.5:
        report_lines.append(
            f"- **Strong negative correlation** between IC and Sharpe (r={overall_ic_corr:.2f})"
        )
        report_lines.append(
            "- Counterintuitively, higher IC leads to worse performance"
        )
    else:
        report_lines.append(
            f"- **Weak correlation** between IC and Sharpe (r={overall_ic_corr:.2f})"
        )
        report_lines.append(
            "- Predictive power does not strongly predict realized performance"
        )
    ...
    verdict_parts.append("")
    verdict_parts.append(
        f"**Recommended Configuration**: {best_overall['signal']} {best_overall['lookback']:.0f}M"
    )
    verdict_parts.append("")
    ...
    if winner == "MOMENTUM":
        report_lines.append(
            "1. **Test dual-momentum** - relative strength + absolute momentum"
        )
        report_lines.append(
            "2. **Test momentum with vol scaling** - scale exposure by realized vol"
        )
        report_lines.append(
            "3. **Test momentum with trend filter** - only trade when above SMA"
        )
    else:
        report_lines.append(
            "1. **Test mean-reversion with RSI** - add overbought/oversold filter"
        )
        report_lines.append(
            "2. **Test mean-reversion with vol filter** - trade more after spikes"
        )
        report_lines.append(
            "3. **Test mean-reversion with regime** - mean-rev in ranging markets only"
        )

    report_lines.append("4. **Test ensemble** - combine multiple lookbacks")
    report_lines.append(
        "5. **Add volatility regime** - scale risk_on by vol percentile"
    )
