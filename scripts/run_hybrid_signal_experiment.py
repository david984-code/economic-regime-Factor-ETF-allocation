...
def _compute_combined_signal_ic(
    ...
    if len(df) < 10:
        return {
            "combined_ic_1m": np.nan,
            "combined_ic_3m": np.nan,
            "combined_ic_6m": np.nan,
            "macro_ic_1m": np.nan,
            "macro_ic_3m": np.nan,
            "macro_ic_6m": np.nan,
            "mr_ic_1m": np.nan,
...
def main():
    ...
    sharpe_diff = (
        hybrid_overall["Strategy_Sharpe"] - baseline_overall["Strategy_Sharpe"]
    )
    maxdd_diff = hybrid_overall["Strategy_MaxDD"] - baseline_overall["Strategy_MaxDD"]
    turnover_diff = hybrid_overall.get("Strategy_Turnover", 0.0) - baseline_overall.get(
    ...
    if ic_stats["combined_ic_avg"] > ic_stats["macro_ic_avg"] + 0.02:
        report_lines.append(
            f"- **Combined signal IC improved by {ic_improvement:.4f}** (meaningful improvement)"
        )
    elif ic_stats["combined_ic_avg"] > ic_stats["macro_ic_avg"]:
        report_lines.append(
            f"- Combined signal IC improved by {ic_improvement:.4f} (marginal improvement)"
        )
    else:
        report_lines.append(
            f"- Combined signal IC did not improve (change: {ic_improvement:.4f})"
        )
    ...
    if abs(ic_stats["mr_ic_avg"]) > abs(ic_stats["macro_ic_avg"]) * 1.5:
        report_lines.append(
            "- **Mean-reversion signal is substantially stronger than macro_score**"
        )
        ...
        report_lines.append(
            "- **Mean-reversion and macro_score have similar strength**"
        )
    ...
    if cagr_diff > 0.01 and sharpe_diff > 0.05:
        ...
        ic_stats["combined_ic_avg"] > ic_stats["macro_ic_avg"] + 0.02 and cagr_diff > 0
    ):
    ...
    if "ACCEPT" in verdict:
        report_lines.append(
            "**Recommendation**: Adopt hybrid signal model for improved robustness."
        )
        report_lines.append("")
        report_lines.append("Next steps:")
        report_lines.append(
            "- Test alternative weight combinations (e.g., 0.3 macro / 0.7 market)"
        )
        report_lines.append(
            "- Consider adding volatility regime or trend strength to the hybrid"
        )
    elif "MARGINAL" in verdict or "NEUTRAL" in verdict:
        report_lines.append(
            "**Recommendation**: Test alternative market signals before deciding."
        )
        report_lines.append("")
        ...
        report_lines.append(
            "- Different weight combinations (e.g., 0.3/0.7 instead of 0.5/0.5)"
        )
    else:
        report_lines.append(
            "**Recommendation**: The mean-reversion signal may be too noisy or poorly timed."
        )
        report_lines.append("")
        report_lines.append("Alternative approaches:")
        report_lines.append("- Test different lookback periods (6M, 18M) for momentum")
        report_lines.append(
            "- Test trend-following (positive momentum) instead of mean-reversion"
        )
