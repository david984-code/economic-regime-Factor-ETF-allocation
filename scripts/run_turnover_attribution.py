...
def main():
    ...
    for t_curr in rebalance_dates[1:]:
        ...
        asset_v_contrib = {
            a: float(abs(w_curr.get(a, 0) - w_cf.get(a, 0))) for a in ASSETS
        }
    ...
    print("  Note: Netting <= 0 means signal and vol changes offset each other.")
    ...
    print(
        f"  {'Year':6} {'Total':>9} {'Signal':>9} {'VolReb':>9} {'Netting':>9} {'%Signal':>8} {'%VolReb':>8}"
    )
    print("  " + "-" * 64)
    for yr, row in by_year.iterrows():
        ...
        print(
            f"  {yr:6} {_fmt(tot):>9} {_fmt(s):>9} {_fmt(v):>9} {_fmt(i):>9} {pct_s:>8.1%} {pct_v:>8.1%}"
        )
    ...
    print(
        f"  Months |delta_ro| < 0.005 (near-zero signal moves): {very_small}  ({very_small / n:.1%})"
    )
    print(
        f"  Months |delta_ro| >= 0.020 (meaningful signal moves): {meaningful}  ({meaningful / n:.1%})"
    )
    print()
    ...
    print(
        f"  Theoretical signal TO (2*|Δrisk_on|*12, ignoring vol scaling): {_fmt(theoretical_s)}"
    )
    print(f"  Actual signal TO from reconstruction: {_fmt(ann_S)}")
    print(
        f"  Difference (vol-scaling interaction on signal): {_fmt(ann_S - theoretical_s, sign=True) if not np.isnan(theoretical_s) else 'n/a'}"
    )
    ...
    print("  Per-asset annual contribution to Vol-rebalancing TO (TO_V):")
    ...
    print(
        f"  Risk-off sleeve TO_V total: {_fmt(tv_rof)}  ({tv_rof / ann_V:.1%} of TO_V)"
    )
    ...
    print(
        f"  Months with meaningful cancellation (I < -0.005): {len(meaningful_cancel)}  ({len(meaningful_cancel) / n:.1%})"
    )
    print(
        f"  Months with near-zero netting (|I| < 0.001):      {len(near_zero_cancel)}  ({len(near_zero_cancel) / n:.1%})"
    )
    print()
    print(
        f"  Average netting per month:  {df['TO_I'].mean():.5f}  ({df['TO_I'].mean() * 12:.3%} annualized)"
    )
    print(f"  Max cancellation in one month: {df['TO_I'].min():.5f}  (most negative)")
    print(
        f"  Max reinforcement in one month: {df['TO_I'].max():.5f}  (should be <= 0 or ~0)"
    )
    print()
    if df["TO_I"].max() > 1e-8:
        ...
        print(
            f"  Worst case: {worst.strftime('%Y-%m')}  TO_I = {df.loc[worst, 'TO_I']:.6f}"
        )
    else:
        print(
            "  Triangle inequality holds: TO_I <= 0 in all months. Reconstruction is clean."
        )
    ...
    weights_df_daily = weights_df_recon.reindex(full_daily_idx, method="ffill").dropna(
        how="all"
    )
    ...
    if discrepancy < 0.005:
        ...
        print(
            "  ACCEPTABLE: discrepancy < 2.0%. Minor edge effects (startup/boundary)."
        )
    else:
    ...
    print("  Walk-forward TO = mean of per-segment turnovers, which can differ.")
