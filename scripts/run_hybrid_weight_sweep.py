...
def _run_experiment(macro_weight: float) -> pd.DataFrame:
    ...
    logger.info("=" * 80)
    logger.info("RUNNING: Macro weight = %.1f / Market weight = %.1f", macro_weight, 1 - macro_weight)
    logger.info("=" * 80)
...
def main():
    ...
    report_lines.append(f"**Best IC**: {best_ic['macro_weight']:.1f}/{best_ic['market_weight']:.1f} (avg IC = {best_ic['ic_avg']:.4f})")
    report_lines.append("")
    ...
    if cagr_vs_market > 0.5:
        report_lines.append("- **CAGR increases with higher market weight** (correlation: {:.2f})".format(cagr_vs_market))
    elif cagr_vs_market < -0.5:
        report_lines.append("- **CAGR increases with higher macro weight** (correlation: {:.2f})".format(-cagr_vs_market))
    else:
        report_lines.append("- CAGR shows no clear trend with weight changes (correlation: {:.2f})".format(cagr_vs_market))
    
    if sharpe_vs_market > 0.5:
        report_lines.append("- **Sharpe increases with higher market weight** (correlation: {:.2f})".format(sharpe_vs_market))
    elif sharpe_vs_market < -0.5:
        report_lines.append("- **Sharpe increases with higher macro weight** (correlation: {:.2f})".format(-sharpe_vs_market))
    else:
        report_lines.append("- Sharpe shows no clear trend with weight changes (correlation: {:.2f})".format(sharpe_vs_market))
    
    if ic_vs_market > 0.5:
        report_lines.append("- **IC increases with higher market weight** (correlation: {:.2f})".format(ic_vs_market))
    elif ic_vs_market < -0.5:
        report_lines.append("- **IC increases with higher macro weight** (correlation: {:.2f})".format(-ic_vs_market))
    else:
        report_lines.append("- IC shows no clear trend with weight changes (correlation: {:.2f})".format(ic_vs_market))
    ...
    if abs(turnover_vs_market) > 0.5:
        ...
        report_lines.append(f"- Turnover {direction} with market weight (correlation: {turnover_vs_market:.2f})")
    else:
        report_lines.append(f"- Turnover is relatively stable across weights (correlation: {turnover_vs_market:.2f})")
    ...
    for _, row in results_df.iterrows():
        report_lines.append(f"| {row['macro_weight']:.1f} | {row['market_weight']:.1f} | {row['robustness_score']:.3f} |")
    
    report_lines.append("")
    report_lines.append(f"**Most Robust**: {best_robust['macro_weight']:.1f}/{best_robust['market_weight']:.1f}")
    report_lines.append("")
    ...
    report_lines.append(f"**Recommended weight: {best_robust['macro_weight']:.1f} macro / {best_robust['market_weight']:.1f} market**")
    report_lines.append("")
    ...
    report_lines.append(f"1. **Performance sensitivity**: CAGR range = {cagr_range:.2%}, Sharpe range = {sharpe_range:.3f}")
    ...
    if cagr_range < 0.005 and sharpe_range < 0.02:
        ...
        report_lines.append("   - Performance shows moderate sensitivity to weight choice")
    ...
    if ic_range < 0.02:
        ...
        report_lines.append("   - Signal strength varies meaningfully with weight choice")
    ...
    if turnover_range > 50:
        report_lines.append("   - Weight choice has substantial impact on trading costs")
    else:
    ...
    if best_robust["macro_weight"] == min(weights):
        report_lines.append(f"4. **Lower macro weights may be even better** - consider testing 0.1 or 0.0 (pure market)")
    elif best_robust["macro_weight"] == max(weights):
        report_lines.append(f"4. **Higher macro weights may be even better** - consider testing 0.9 or 1.0 (pure macro)")
    else:
        report_lines.append(f"4. **Optimal weight is in the interior** - balanced combination is best")
