...
def _build_diagnostics(prices: pd.DataFrame, regime_df: pd.DataFrame) -> dict:
    ...
    for i in range(len(spy_monthly)):
        ...
        signal_records.append({"date": spy_monthly.index[i], "momentum_24m": mom, "risk_on_raw": 1 if mom > 0 else 0})
    ...
    for date, val in flips.items():
        prev_val = signal_clean.shift(1).loc[date]
        direction = "risk_off -> risk_on" if val == 1 else "risk_on -> risk_off"
        mom_val = signal_df.loc[date, "momentum_24m"] if "momentum_24m" in signal_df.columns else np.nan
        log_a.append({
    ...
    for d in month_changed_dates:
        ...
        if len(me_date) < 24:
            continue
        last_me = me_date[-1]
        sig = signal_df.loc[last_me, "risk_on_raw"] if last_me in signal_df.index else np.nan
        log_b.append({
            "execution_date": d.strftime("%Y-%m-%d"),
            "baseline_executes": "YES",
            "experiment_executes": "YES" if is_q else "NO (held)",
            "risk_on_signal": f"{sig:.0f}" if not (isinstance(sig, float) and np.isnan(sig)) else "n/a",
        })
    ...
    for rec in log_b:
        ...
        sig = int(rec["risk_on_signal"]) if rec["risk_on_signal"] not in ("n/a", "") else None
        if sig is None:
            continue
        ...
        if rec["experiment_executes"] == "YES":
            ...
            if divergence_open is not None:
                ...
                log_c.append({
                    ...
                    "experiment_risk_on": f"{last_q_signal:.0f}" if last_q_signal is not None else "n/a",
...
def _march_2020_timeline(log_b: list, signal_df: pd.DataFrame) -> dict:
    ...
    signal_flip_date = flip_2020.index[0].strftime("%Y-%m-%d") if len(flip_2020) > 0 else "no flip in window"
...
def _2022_risk_off(log_b: list, signal_df: pd.DataFrame) -> dict:
    ...
    flip_date = flip_2022.index[0].strftime("%Y-%m-%d") if len(flip_2022) > 0 else "no flip"
...
def main():
    ...
    to_delta     = to_exp - to_base if not (np.isnan(to_base) or np.isnan(to_exp)) else float("nan")
    ...
    print("-" * 66)
    print(f"{'CAGR':30} {b['Strategy_CAGR']:>12.2%} {e['Strategy_CAGR']:>12.2%} {cagr_delta:>+10.2%}")
    print(f"{'Sharpe':30} {b['Strategy_Sharpe']:>12.3f} {e['Strategy_Sharpe']:>12.3f} {sharpe_delta:>+10.3f}")
    print(f"{'MaxDD':30} {b['Strategy_MaxDD']:>12.2%} {e['Strategy_MaxDD']:>12.2%} {maxdd_delta:>+10.2%}")
    print(f"{'Vol':30} {b['Strategy_Vol']:>12.2%} {e['Strategy_Vol']:>12.2%} {vol_delta:>+10.2%}")
    ...
    diff_sharpe_b = seg_base.loc[diff_mask_b, "Strategy_Sharpe"].mean() if diff_mask_b.any() else float("nan")
    diff_sharpe_e = seg_exp.loc[diff_mask_e,  "Strategy_Sharpe"].mean() if diff_mask_e.any() else float("nan")
    difficult_improved = (not np.isnan(diff_sharpe_e) and not np.isnan(diff_sharpe_b)
                          and diff_sharpe_e > diff_sharpe_b)
    ks_no_difficult = not difficult_improved

    kill = ks_sharpe and ks_cagr and ks_no_difficult
    print(f"  Sharpe delta < +0.02?         {'YES' if ks_sharpe else 'NO':4}  (delta = {sharpe_delta:+.3f})")
    print(f"  CAGR delta < +0.25%?          {'YES' if ks_cagr else 'NO':4}  (delta = {cagr_delta:+.2%})")
    print(f"  No difficult-period impr.?    {'YES' if ks_no_difficult else 'NO':4}  "
    ...
    for row in diag["log_a"]:
        print(f"  {row['month_end_date']:15} {row['direction']:25} {row['momentum_24m']:>14}")

    # Log B
    print("\nLOG B: REBALANCE EXECUTION DATES")
    print("-" * 66)
    print(f"  {'Exec date':12} {'Baseline':12} {'Experiment':20} {'Signal risk_on':>14}")
    for row in diag["log_b"]:
        print(f"  {row['execution_date']:12} {row['baseline_executes']:12} "
    ...
    if diag["log_c"]:
        for row in diag["log_c"]:
            print(f"  {row['period_start']} -> {row['period_end']}: "
                  f"baseline risk_on={row['baseline_risk_on']}, "
                  f"experiment held={row['experiment_risk_on']}  [{row['note']}]")
    else:
        print("  No allocation divergences detected (signal did not change between quarters).")

    # March 2020
    print("\nMARCH 2020 TIMELINE")
    print("-" * 66)
    print(f"  Signal flip to risk-off (month-end): {m2020['signal_flip_date']}")
    print(f"  Baseline first risk-off execution:   {m2020['baseline_first_risk_off_exec']}")
    print(f"  Experiment first risk-off execution: {m2020['experiment_first_risk_off_exec']}")
    ...
    esc_turnover = (not np.isnan(to_delta) and to_delta <= -0.40 * to_base and sharpe_delta >= -0.02)
    esc_difficult = difficult_improved
    escalate = esc_sharpe or esc_turnover or esc_difficult
    print(f"  Sharpe delta ≥ +0.02?                      {'YES' if esc_sharpe else 'NO'}")
    print(f"  Turnover reduction ≥ 40%, Sharpe ≥ -0.02? {'YES' if esc_turnover else 'NO'}")
    print(f"  Difficult-period improvement?              {'YES' if esc_difficult else 'NO'}")
