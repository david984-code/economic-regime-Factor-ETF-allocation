...
def main():
    ...
    baseline_difficult_cagr = results_df[results_df["filter_type"] == "none"].iloc[0][
        "difficult_cagr"
    ]
    baseline_difficult_sharpe = results_df[results_df["filter_type"] == "none"].iloc[0][
        "difficult_sharpe"
    ]
    ...
    for _, row in results_df.iterrows():
        ...
        overall_better = (
            row["sharpe"] > baseline["sharpe"] * 1.01
        )  # 1% improvement threshold
        difficult_better = (
            row["difficult_sharpe"] > baseline_difficult_sharpe * 1.05
    ...
    for _, row in results_df.iterrows():
        report_lines.append(
            f"| {row['filter']:>20} | {row['robustness_score']:>5.3f} |"
        )
    ...
    best_filter = results_df[results_df["filter_type"] != "none"].loc[
        results_df[results_df["filter_type"] != "none"]["sharpe"].idxmax()
    ]
    ...
    if best_filter["sharpe"] > baseline_sharpe * 1.02:  # 2% improvement threshold
        ...
        best_difficult_sharpe["filter_type"] != "none"
        and best_difficult_sharpe["difficult_sharpe"] > baseline_difficult_sharpe * 1.1
    ):  # 10% improvement in difficult periods
    ...
    if best_filter["sharpe"] <= baseline_sharpe * 1.02:
        ...
        avg_filter_sharpe = results_df[results_df["filter_type"] != "none"][
            "sharpe"
        ].mean()
        ...
        if avg_filter_sharpe < baseline_sharpe:
            report_lines.append(
                f"- Filters reduce Sharpe on average (avg: {avg_filter_sharpe:.3f} vs baseline: {baseline_sharpe:.3f})"
            )
        if avg_filter_cagr < baseline["cagr"]:
            report_lines.append(
                f"- Filters reduce CAGR on average (avg: {avg_filter_cagr:.2%} vs baseline: {baseline['cagr']:.2%})"
            )

        report_lines.append(
            "- 24M momentum already captures long-term trends effectively"
        )
        report_lines.append(
            "- Additional filtering introduces false signals or whipsaws"
        )
        report_lines.append("")
    ...
    if best_filter["sharpe"] > baseline_sharpe * 1.02:
        report_lines.append(
            "1. **Fine-tune filter threshold** - test different risk_on caps (0.2, 0.4, 0.5)"
        )
        report_lines.append(
            "2. **Test filter combinations** - combine multiple trend signals"
        )
    else:
        report_lines.append(
            "1. **Test volatility scaling** - scale risk_on by realized vol percentile"
        )
        report_lines.append(
            "2. **Test dual-momentum** - add relative strength component"
        )
        report_lines.append("3. **Test ensemble** - combine 12M + 24M momentum signals")

    report_lines.append(
        "4. **Test alternative assets** - expand universe beyond current ETFs"
    )
    report_lines.append(
        "5. **Test regime-conditional momentum** - apply momentum only in trending regimes"
    )
