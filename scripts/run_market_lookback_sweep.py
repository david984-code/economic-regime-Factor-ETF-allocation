...
def main():
    ...
    report_lines.append(
        f"**Best IC**: {best_ic['lookback']:.0f}M (avg IC = {best_ic['ic_avg']:.4f})"
    )
    report_lines.append("")
    ...
    if abs(cagr_vs_lookback) > 0.5:
        ...
        report_lines.append(
            f"- CAGR {direction} with longer lookback (correlation: {cagr_vs_lookback:.2f})"
        )
    else:
        report_lines.append(
            f"- CAGR shows no clear trend with lookback (correlation: {cagr_vs_lookback:.2f})"
        )

    if abs(sharpe_vs_lookback) > 0.5:
        direction = "increases" if sharpe_vs_lookback > 0 else "decreases"
        report_lines.append(
            f"- Sharpe {direction} with longer lookback (correlation: {sharpe_vs_lookback:.2f})"
        )
    else:
        report_lines.append(
            f"- Sharpe shows no clear trend with lookback (correlation: {sharpe_vs_lookback:.2f})"
        )

    if abs(ic_vs_lookback) > 0.5:
        direction = "increases" if ic_vs_lookback > 0 else "decreases"
        report_lines.append(
            f"- IC {direction} with longer lookback (correlation: {ic_vs_lookback:.2f})"
        )
    else:
        report_lines.append(
            f"- IC shows no clear trend with lookback (correlation: {ic_vs_lookback:.2f})"
        )

    if abs(turnover_vs_lookback) > 0.5:
        direction = "increases" if turnover_vs_lookback > 0 else "decreases"
        report_lines.append(
            f"- Turnover {direction} with longer lookback (correlation: {turnover_vs_lookback:.2f})"
        )
    else:
        report_lines.append(
            f"- Turnover shows no clear trend with lookback (correlation: {turnover_vs_lookback:.2f})"
        )
    ...
    report_lines.append("### Sensitivity to Lookback Choice")
    report_lines.append("")
    ...
    if cagr_range < 0.005 and sharpe_range < 0.02:
        report_lines.append(
            "**Low sensitivity**: Performance is stable across lookback choices."
        )
    elif cagr_range > 0.02:
        report_lines.append(
            "**High sensitivity**: Lookback choice has substantial impact on performance."
        )
    else:
        report_lines.append(
            "**Moderate sensitivity**: Lookback choice has noticeable but not dramatic impact."
        )
    ...
    for _, row in results_df.iterrows():
        report_lines.append(
            f"| {row['lookback']:>8}M | {row['robustness_score']:>5.3f} |"
        )
    ...
    ic_df[["ic_1m", "ic_3m", "ic_6m"]].apply(lambda col: (col > 0).sum())
    ...
    if (ic_df["ic_avg"] < 0).all():
        ...
        report_lines.append(
            "- Some lookbacks show mean reversion, others show momentum"
        )
    ...
    if best_robust["lookback"] <= 6:
        report_lines.append(
            f"Best lookback ({best_robust['lookback']:.0f}M) is SHORT-TERM:"
        )
        ...
        report_lines.append(
            f"Best lookback ({best_robust['lookback']:.0f}M) is LONG-TERM:"
        )
        ...
        report_lines.append(
            f"Best lookback ({best_robust['lookback']:.0f}M) is MEDIUM-TERM:"
        )
    ...
    if best_robust["lookback"] == 12:
        ...
        explanation = (
            "The 12-month lookback provides the best risk-adjusted performance."
        )
    else:
        verdict = f"CHANGE TO {best_robust['lookback']:.0f}M: Improves on current 12M baseline."
        cagr_improvement = (
            best_robust["cagr"]
            - results_df[results_df["lookback"] == 12].iloc[0]["cagr"]
        )
        sharpe_improvement = (
            best_robust["sharpe"]
            - results_df[results_df["lookback"] == 12].iloc[0]["sharpe"]
        )
    ...
    if ic_range > 0.05:
        report_lines.append(
            "1. **Test ensemble of multiple lookbacks** - IC varies significantly, combining may help"
        )

    if (ic_df["ic_avg"] > 0).any() and (ic_df["ic_avg"] < 0).any():
        report_lines.append(
            "2. **Test trend-following vs mean-reversion** - signal switches behavior by horizon"
        )

    if best_robust["turnover"] > 150:
        report_lines.append(
            "3. **Add turnover constraint or smoothing** - high turnover erodes returns"
        )

    report_lines.append(
        "4. **Add volatility regime filter** - scale risk_on by realized vol"
    )
    report_lines.append(
        "5. **Test alternative market signals** - trend strength, breadth, vol percentile"
    )
