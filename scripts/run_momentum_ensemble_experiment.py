...
def _run_experiment(
    ...
    if momentum_12m_weight == 0.0:
        ...
        logger.info("=" * 80)
        logger.info("RUNNING: %.0f%% 12M + %.0f%% %dM ensemble", 
...
def _compute_signal_ic(
    ...
    if momentum_12m_weight > 0:
        ...
        blended_signal = (1 - momentum_12m_weight) * primary_signal + momentum_12m_weight * momentum_12m_signal
    else:
    ...
    for i in range(len(blended_signal)):
        ...
        if len(trailing) >= min_history:
            signal_z.iloc[i] = (blended_signal.iloc[i] - trailing.mean()) / trailing.std()
        else:
            signal_z.iloc[i] = 0.0
...
def main():
    ...
    report_lines.append(f"**Best IC**: {best_ic['signal']} (avg IC = {best_ic['ic_avg']:.4f})")
    report_lines.append("")
    ...
    if len(ensemble_signals) > 0:
        ...
        if avg_ensemble_sharpe > avg_single_sharpe:
            ...
            report_lines.append(f"**Ensemble improves Sharpe by {improvement:+.3f} on average**")
        else:
            degradation = avg_single_sharpe - avg_ensemble_sharpe
            report_lines.append(f"**Ensemble reduces Sharpe by {degradation:.3f} on average**")
    ...
    if momentum_12m_only["sharpe"] > baseline["sharpe"]:
        report_lines.append(f"**12M outperforms 24M**: Shorter horizon is better")
        report_lines.append(f"- 12M Sharpe: {momentum_12m_only['sharpe']:.3f}")
        report_lines.append(f"- 24M Sharpe: {baseline['sharpe']:.3f}")
        report_lines.append(f"- Improvement: {momentum_12m_only['sharpe'] - baseline['sharpe']:+.3f}")
    else:
        report_lines.append(f"**24M outperforms 12M**: Longer horizon is better")
        report_lines.append(f"- 24M Sharpe: {baseline['sharpe']:.3f}")
        report_lines.append(f"- 12M Sharpe: {momentum_12m_only['sharpe']:.3f}")
        report_lines.append(f"- Advantage: {baseline['sharpe'] - momentum_12m_only['sharpe']:+.3f}")
    ...
    for _, row in results_df.iterrows():
        report_lines.append(f"| {row['signal']:>18} | {row['robustness_score']:>5.3f} |")
    ...
    if best_signal["signal"] == "24M only":
        ...
        avg_ensemble_sharpe = ensemble_signals["sharpe"].mean() if len(ensemble_signals) > 0 else 0
        avg_ensemble_turnover = ensemble_signals["turnover"].mean() if len(ensemble_signals) > 0 else 0
        
        if avg_ensemble_sharpe < baseline_sharpe:
            report_lines.append(f"- Ensembles reduce Sharpe on average (avg: {avg_ensemble_sharpe:.3f} vs baseline: {baseline_sharpe:.3f})")
        if avg_ensemble_turnover > baseline["turnover"]:
            report_lines.append(f"- Ensembles increase turnover (avg: {avg_ensemble_turnover:.1%} vs baseline: {baseline['turnover']:.1%})")
        
        report_lines.append("- 24M momentum captures long-term trends effectively")
        report_lines.append("- Adding shorter horizons introduces noise without benefit")
        report_lines.append("")
    ...
    if best_signal["signal"] != "24M only":
        report_lines.append("1. **Fine-tune ensemble weights** - test more granular blends")
        report_lines.append("2. **Test adaptive ensemble** - vary weights based on market conditions")
    else:
        report_lines.append("1. **Expand asset universe** - test sector rotation, international, alternatives")
        report_lines.append("2. **Test asset-specific momentum** - apply momentum to individual assets, not just market")
        report_lines.append("3. **Test regime-conditional signals** - vary signal by macro regime")
    
    report_lines.append("4. **Test concentration limits** - constrain max position size in equal weight")
    report_lines.append("5. **Test dynamic universe** - vary assets based on momentum signal strength")
