...
def _forward_returns(ret: pd.Series, horizon: int) -> pd.Series:
    ...
    fwd = (1 + ret).rolling(horizon).apply(lambda x: x.prod() - 1, raw=True).shift(-horizon)
    return fwd
...
def _bucket_statistics(
    ...
    df["bucket"] = pd.qcut(df["macro_score"], q=n_buckets, labels=False, duplicates="drop")
    
    stats = df.groupby("bucket").agg({
        "macro_score": ["mean", "min", "max", "count"],
        "fwd_ret": ["mean", "std"],
    })
    stats.columns = ["macro_score_mean", "macro_score_min", "macro_score_max", "n_obs", "fwd_ret_mean", "fwd_ret_std"]
...
def main() -> None:
    ...
    if not stats_1m.empty:
        report_lines.append("| Bucket | Macro Score Range | N | Avg Fwd Ret | Vol | Sharpe |")
        report_lines.append("|--------|-------------------|---|-------------|-----|--------|")
        for _, row in stats_1m.iterrows():
            ...
            vol = row["fwd_ret_std"]
            sharpe = row["sharpe"]
            report_lines.append(f"| {bucket} | {ms_range} | {n} | {fwd_ret:.2%} | {vol:.2%} | {sharpe:.3f} |")
        report_lines.append("")
    ...
    if ic_stability_1m > 0.2 or ic_stability_3m > 0.2:
        report_lines.append("**Stability**: High volatility in IC suggests signal is unstable over time.")
    elif (rolling_ic_1m > 0).mean() > 0.6 and (rolling_ic_3m > 0).mean() > 0.6:
        report_lines.append("**Stability**: IC consistently positive, suggesting stable predictive power.")
    else:
        report_lines.append("**Stability**: Mixed IC sign suggests weak or inconsistent signal.")
    report_lines.append("")
    ...
    if abs(macro_ic_avg) < 0.03:
        report_lines.append("- **Macro score has near-zero predictive power** (avg IC < 0.03)")
    elif abs(macro_ic_avg) < 0.05:
        report_lines.append("- **Macro score has weak predictive power** (avg IC < 0.05)")
    elif abs(macro_ic_avg) < 0.10:
        report_lines.append("- **Macro score has moderate predictive power** (avg IC between 0.05-0.10)")
    else:
        report_lines.append("- **Macro score has strong predictive power** (avg IC > 0.10)")
    ...
    monotonic_1m = not stats_1m.empty and (stats_1m["fwd_ret_mean"].diff().dropna() > 0).all()
    monotonic_3m = not stats_3m.empty and (stats_3m["fwd_ret_mean"].diff().dropna() > 0).all()
    monotonic_6m = not stats_6m.empty and (stats_6m["fwd_ret_mean"].diff().dropna() > 0).all()
    
    if monotonic_1m and monotonic_3m and monotonic_6m:
        report_lines.append("- **Higher macro_score consistently predicts higher forward returns** across all horizons")
    elif monotonic_1m or monotonic_3m or monotonic_6m:
        report_lines.append("- **Partial monotonicity** observed at some horizons")
    else:
        report_lines.append("- **Non-monotonic relationship** between macro_score and forward returns")
    ...
    report_lines.append(f"- **Best signal**: {best_signal[0]} (avg IC = {best_signal[1]:.4f})")
    report_lines.append(f"- Macro score vs Momentum: {macro_ic_avg:.4f} vs {momentum_ic_avg:.4f}")
    report_lines.append(f"- Macro score vs SMA Trend: {macro_ic_avg:.4f} vs {sma_ic_avg:.4f}")
    report_lines.append("")
    
    if abs(momentum_ic_avg) > abs(macro_ic_avg) * 1.5:
        report_lines.append("- **Simple momentum is substantially stronger than macro_score**")
    elif abs(sma_ic_avg) > abs(macro_ic_avg) * 1.5:
        report_lines.append("- **Simple trend signal is substantially stronger than macro_score**")
    elif abs(macro_ic_avg) > max(abs(momentum_ic_avg), abs(sma_ic_avg)) * 1.2:
        report_lines.append("- **Macro score provides unique information beyond simple market signals**")
    else:
        report_lines.append("- **Macro score has similar strength to simple market signals**")
    ...
    if (rolling_ic_1m > 0).mean() > 0.65:
        report_lines.append("- **1M IC is consistently positive**, suggesting reliable short-term signal")
    ...
    if (rolling_ic_3m > 0).mean() > 0.65:
        report_lines.append("- **3M IC is consistently positive**, suggesting reliable medium-term signal")
    ...
    if abs(macro_ic_avg) > 0.05 and (rolling_ic_1m > 0).mean() > 0.6:
        ...
        recommendation = "Test whether combining macro_score with market signals improves results."
    else:
