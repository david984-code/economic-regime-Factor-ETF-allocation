...
def main() -> None:
    ...
    for _, row in stag_df.iterrows():
        ...
        spy_ret = sub_rets["SPY"] if "SPY" in sub_rets else pd.Series(0, index=sub_rets.index)
        ...
        if risk_on_off_ret is not None:
            ...
            rof_sub = 0.33 * sub_rets["IEF"] + 0.33 * sub_rets["TLT"] + 0.34 * sub_rets["GLD"]
        spy_m = _metrics_from_rets(spy_ret)
        rof_m = _metrics_from_rets(rof_sub)

        opt_weights_list.append({**stag_alloc, "test_start": row["test_start"], "test_end": row["test_end"]})
        seg_data.append({
    ...
    print("=" * 80)
    weight_cols = [c for c in opt_weights_df.columns if c not in ("test_start", "test_end")]
    ...
    print(opt_weights_df[["test_start", "test_end"] + [c for c in weight_cols if opt_weights_df[c].sum() > 0][:8]].head(10).to_string(index=False))
    ...
    for a, c in sorted(contrib.items(), key=lambda x: -abs(x[1])):
        print(f"  {a:>6}: {c:.2%} (weight {avg_weights[a]:.1%} x return {asset_cagrs[a]:.1%})")
    ...
    if len(wmat) >= 2:
        ...
        print(f"  Avg |weight change| when rolling to next segment: {avg_turnover_per_step:.1%}")

    # 7. Segment period distribution (early vs late)
    print("\n" + "=" * 80)
    print("7. SEGMENT PERIOD DISTRIBUTION")
    print("=" * 80)
    seg_df["period"] = seg_df["test_start"].apply(lambda x: "pre-2019" if x < "2019" else "2019+")
    early = seg_df[seg_df["period"] == "pre-2019"]
    late = seg_df[seg_df["period"] == "2019+"]
    print(f"  Pre-2019 segments: {len(early)}, Optimizer CAGR: {early['opt_cagr'].mean():.2%}, Alt1 CAGR: {early['alt1_cagr'].mean():.2%}")
    print(f"  2019+ segments: {len(late)}, Optimizer CAGR: {late['opt_cagr'].mean():.2%}, Alt1 CAGR: {late['alt1_cagr'].mean():.2%}" if len(late) > 0 else "  2019+ segments: 0")

    # 8. Diagnosis
    print("\n" + "=" * 80)
    print("8. DIAGNOSIS & RECOMMENDATION")
    print("=" * 80)
    best_alt = max([("Alt1", seg_df["alt1_cagr"].mean()), ("Alt2", seg_df["alt2_cagr"].mean()),
                    ("Alt3", seg_df["alt3_cagr"].mean())], key=lambda x: x[1])
    print(f"  Best alternative: {best_alt[0]} (CAGR {best_alt[1]:.2%})")
    print(f"  Optimizer Stagflation: CAGR {seg_df['opt_cagr'].mean():.2%}, Sharpe {seg_df['opt_sharpe'].mean():.3f}")

    # Write markdown report
    _write_report(opt_weights_df, seg_df, avg_weights, std_weights, asset_cagrs, contrib, best_alt)
...
def _write_report(
    ...
    for a in sorted(avg_weights.index, key=lambda x: -avg_weights[x]):
        if avg_weights[a] > 0.001:
            lines.append(f"| {a} | {avg_weights[a]:.1%} | {std_weights.get(a, 0):.1%} |")
    lines.append("")
    lines.append("---")
    lines.append("")
    lines.append("## 3. Alternative Allocations and Performance (Stagflation segments only)")
    lines.append("")
    alts = [
        ("Optimizer", "opt"),
        ("Alt1: 30% GLD, 40% IEF, 20% TLT, 10% cash", "alt1"),
    ...
    for name, key in alts:
        cagr = seg_df[f"{key}_cagr"].mean()
        sharpe = seg_df[f"{key}_sharpe"].mean()
        maxdd = seg_df[f"{key}_maxdd"].mean() if f"{key}_maxdd" in seg_df.columns else np.nan
        vol = seg_df[f"{key}_vol"].mean() if f"{key}_vol" in seg_df.columns else np.nan
        lines.append(f"| {name} | {cagr:.2%} | {sharpe:.3f} | {maxdd:.2%} | {vol:.2%} |")
    lines.append("")
    lines.append("---")
    lines.append("")
    lines.append("## 4. Why the Optimizer Chooses This Mix")
    lines.append("")
    lines.append("- **Objective:** Sortino (return / downside vol). Favors assets with low downside volatility in training Stagflation months.")
    lines.append("- **Historical periods:** Training uses expanding window; Stagflation months are sparse. Early train periods (2010-2016) had few Stagflation months; IJR may have had favorable Sortino in those.")
    lines.append("- **Constraints:** GLD min 8% (REGIME_MIN_ASSETS), cash 10-20%. No IEF min, no IJR cap.")
    lines.append("- **IJR role:** IJR gets ~49% weight because it had strong Sortino in training Stagflation months. In this sample, IJR returned 17.3% during Stagflation segments (contribution +8.56%). TLT (-4.0% return) is a drag (-0.93% contrib) but optimizer still holds 23%.")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    seg_df_copy["period"] = seg_df_copy["test_start"].apply(lambda x: "pre-2019" if x < "2019" else "2019+")
    ...
    lines.append("|--------|------------|---------------|------------|")
    lines.append(f"| pre-2019 | {len(early)} | {early['opt_cagr'].mean():.2%} | {early['alt1_cagr'].mean():.2%} |")
    if len(late) > 0:
        lines.append(f"| 2019+ | {len(late)} | {late['opt_cagr'].mean():.2%} | {late['alt1_cagr'].mean():.2%} |")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append("|------|--------|-----------|")
    lines.append("| 1 | **Recalibrate risk_on in Stagflation** | Override failed; baseline (risk_on blend) beats optimizer in practice. Cap risk_on at 0.2 when regime=Stagflation so blend is more defensive without changing optimizer. |")
    lines.append("| 2 | **Change optimizer constraints** | Add IEF min (e.g. 10%), cap IJR (e.g. 20%) in Stagflation. TLT is a drag (-0.93% contrib); IEF may help. |")
    lines.append("| 3 | **Introduce regime-specific hardcoded sleeve** | Use 30% GLD, 40% IEF, 20% TLT when Stagflation. Beats optimizer in hostile periods; loses in benign (2015-17). |")
    lines.append("| 4 | **Change asset universe** | Exclude IJR from Stagflation optimization to avoid concentration risk. |")
    lines.append("")
    lines.append("---")
    lines.append("")
    lines.append("")
