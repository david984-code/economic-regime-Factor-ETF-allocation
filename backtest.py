...
def _blend_alloc(w_off: dict, w_on: dict, alpha: float) -> dict:
    ...
    w = {a: (1.0 - alpha) * float(w_off.get(a, 0.0)) + alpha * float(w_on.get(a, 0.0)) for a in ASSETS}
    s = sum(w.values())
    if s <= 0:
    ...
    if (prev_month is None) or (month != prev_month):
        ...
        if 'risk_on' in regime_df.columns and (not pd.isna(regime_df.loc[date, 'risk_on'])):
            ...
            if regime_key in allocations:
                ...
                print(f"⚠️ Warning: Unknown regime label '{regime}' on {date.date()} (keeping previous weights)")
...
def compute_metrics(rets, rf_daily=0.0):
    ...
    print("📈 Mean daily EXCESS return:", mean_daily)
...
print("\n✅ Saved: current_factor_weights.csv")
