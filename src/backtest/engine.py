...
def _equal_weight_alloc(
    ...
    risk_on_assets = (
        risk_on_sleeve if risk_on_sleeve is not None else RISK_ON_ASSETS_BASE
    )
    risk_off_assets = (
        risk_off_sleeve if risk_off_sleeve is not None else RISK_OFF_ASSETS_BASE
    )
        ...
        if filter_method == "positive":
            ...
            if len(selected) == 0:
                sorted_assets = sorted(
                    risk_on_assets,
                    key=lambda a: momentum_row.get(a, -999),
                    reverse=True,
                )
            ...
            sorted_assets = sorted(
                risk_on_assets, key=lambda a: momentum_row.get(a, -999), reverse=True
            )
...
def _risk_parity_alloc(
    ...
    trailing_vol = (
        returns.rolling(window=lookback_days, min_periods=lookback_days).std().iloc[-1]
    )
        ...
        for i in range(len(realized_vol)):
            if pd.isna(realized_vol.iloc[i]):
                ...
                if len(trailing_vols) >= lookback * 2:
                    current_vol = realized_vol.iloc[i]
                    percentile = (trailing_vols < current_vol).sum() / len(
                        trailing_vols
                    )
...
def _compute_volatility_regime_score(
    ...
    for i in range(len(vol_monthly)):
        if pd.isna(vol_monthly.iloc[i]):
            ...
            trailing_vols = vol_monthly.iloc[
                max(0, i + 1 - lookback_months) : i + 1
            ].dropna()
        ...
        if pd.isna(percentile_series.iloc[i]):
            ...
            if len(trailing) >= min_history:
                ...
                if std_pctl > 0:
                    ...
                    vol_regime_score.iloc[i] = (
                        -(percentile_series.iloc[i] - mean_pctl) / std_pctl
                    )
                else:
                    vol_regime_score.iloc[i] = 0.0
            else:
...
def _compute_market_primary_risk_on(
    ...
    for i in range(len(spy_monthly)):
        if i < market_lookback_months:
            ...
            ret = (
                spy_monthly.iloc[i] / spy_monthly.iloc[i - market_lookback_months]
            ) - 1
...
def _compute_hybrid_risk_on(
    ...
    for i in range(len(market_signal_aligned)):
        ...
        if len(trailing) >= min_history:
            market_signal_z.iloc[i] = (
                market_signal_aligned.iloc[i] - trailing.mean()
            ) / trailing.std()
        else:
    ...
    if momentum_6m_weight > 0.0:
        ...
        for i in range(len(mom6_aligned)):
            ...
            if len(trailing) >= min_6m:
                ...
                mom6_z.iloc[i] = (
                    (float(mom6_aligned.iloc[i]) - float(trailing.mean())) / std
                    if std > 1e-10
                    else 0.0
                )
            else:
        ...
        market_signal_z = (
            1.0 - momentum_6m_weight
        ) * market_signal_z + momentum_6m_weight * mom6_z

    # Optionally compute volatility regime score
    if use_vol_regime and vol_regime_weight > 0:
        vol_regime_score = _compute_volatility_regime_score(
            prices, lookback_days=63, percentile_window_years=3
        )
        ...
        combined_score = (
            macro_weight * macro_score_monthly + (1 - macro_weight) * market_signal_z
        )
    ...
    if breadth_weight > 0.0 and breadth_prices is not None:
        ...
        combined_score = (
            1.0 - breadth_weight
        ) * combined_score + breadth_weight * breadth_z
    ...
    if vol_zscore_weight > 0.0:
        ...
        combined_score = (
            1.0 - vol_zscore_weight
        ) * combined_score - vol_zscore_weight * vol_z
    ...
    if yield_curve_weight > 0.0 and yield_curve_data is not None:
        yc_aligned = (
            yield_curve_data.resample("ME").last().reindex(regime_monthly.index)
        )
        ...
        combined_score = (
            1.0 - yield_curve_weight
        ) * combined_score + yield_curve_weight * yc_z
    ...
    regime_daily_hybrid = regime_monthly[["hybrid_risk_on"]].reindex(
        regime_df.index, method="ffill"
    )

    # Replace risk_on with hybrid
    regime_df["risk_on"] = regime_daily_hybrid["hybrid_risk_on"].fillna(
        regime_df["risk_on"]
    )
    ...
    if trend_filter_type != "none":
        ...
        regime_df.loc[~trend_filter_aligned, "risk_on"] = regime_df.loc[
            ~trend_filter_aligned, "risk_on"
        ].clip(upper=trend_filter_risk_on_cap)
...
def _smooth_regime_labels(
    ...
    # Forward-fill to daily
    regime_daily = regime_monthly[["regime_smoothed"]].reindex(
        regime_df.index, method="ffill"
    )
...
def _compute_returns_and_setup(
    ...
    if use_market_primary:
        ...
        regime_df.loc[~trend_aligned, "risk_on"] = regime_df.loc[
            ~trend_aligned, "risk_on"
        ].clip(upper=trend_filter_risk_on_cap)
    ...
    if portfolio_construction_method == "optimizer":
        ...
        momentum_ts = _compute_asset_momentum_timeseries(
            prices, assets_to_use, market_lookback_months
        )
        ...
        raise ValueError(
            f"Unknown portfolio_construction_method: {portfolio_construction_method}"
        )
...
def _run_backtest_loop(
    ...
    current_weights: dict[str, float] = {a: 1.0 / (len(TICKERS) + 1) for a in TICKERS}
    ...
    for date in returns.index:
        ...
        if is_new_month and (not quarterly_rebalance or is_quarter_start):
            ...
            has_dynamic_risk_on = any(
                f"w_risk_on_{a}" in regime_df.columns for a in ASSETS
            )

            if (
                use_stagflation_override
                and regime_stripped == "Stagflation"
                and "Stagflation" in allocations
            ):
                current_weights = {
                    str(k): float(v) for k, v in allocations["Stagflation"].items()
                }
            elif "risk_on" in regime_df.columns and not pd.isna(
                regime_df.loc[date, "risk_on"]
            ):
                ...
                if has_dynamic_risk_on:
                    w_risk_on_monthly = {
                        a: float(regime_df.loc[date, f"w_risk_on_{a}"]) for a in ASSETS
                    }
                    current_weights = _blend_alloc(
                        w_risk_off, w_risk_on_monthly, alpha, ASSETS
                    )
                else:
        ...
        if rebalanced:
            turnover = sum(
                abs(
                    float(current_weights.get(a, 0.0))
                    - float(prev_weights_for_cost.get(a, 0.0))
                )
                for a in ASSETS
            )
...
def _run_backtest_vectorized(
    ...
    ] = []  # (date, scale) when vol_target_annual > 0
    ...
    ret_arr = returns[
        weight_cols
    ].values  # needed for vol targeting and later for portfolio_ret
    ...
    for i, date in enumerate(dates):
        ...
        if month_changed[i]:
            ...
            if (
                return_turnover_attribution
                and prev_start is not None
                and not has_dynamic_risk_on
                and "risk_on" in regime_df.columns
                and not pd.isna(regime_df.loc[date, "risk_on"])
            ):
                alpha = float(regime_df.loc[date, "risk_on"])
                ...
                if use_post_blend_inv_vol and prev_std_dict is not None:
                    ...
                    to_invvol = 0.0
                to_sleeve_internal = (
                    0.0  # equal-weight sleeves: no sleeve-internal change
                )
                valid_vols = [
                    v for v in std_dict.values() if v is not None and not np.isnan(v)
                ]
            ...
            if vol_target_annual > 0 and cash_idx is not None:
                if i >= 63:
                    port_ret_63 = (ret_arr[i - 63 : i] * weights[i - 63 : i]).sum(
                        axis=1
                    )
    ...
    if return_weights:
        ...
        if return_turnover_attribution and attribution_records:
            ...
            return (
                pd.Series(portfolio_ret, index=dates),
                weights_df,
                scale_series,
                att_df,
            )
...
def run_backtest_with_allocations(
    ...
    | tuple[pd.Series, pd.DataFrame]
    | tuple[pd.Series, pd.DataFrame, pd.Series]
    | tuple[pd.Series, pd.DataFrame, pd.Series | None, pd.DataFrame]
):
...
def run_backtest(pipeline_data: "PipelineData | None" = None) -> dict[str, Any]:
    ...
    if asof_regime_stripped == "Stagflation" and "Stagflation" in allocations:
        ...
        if alpha_for_weights is not None:
            base_weights = _blend_alloc(
                w_risk_off, w_risk_on, alpha_for_weights, ASSETS
            )
        else:
