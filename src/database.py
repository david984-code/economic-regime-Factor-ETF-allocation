...
class Database:
    ...
    def _create_tables(self) -> None:
        ...
        # Current weights table (historical tracking)
        cursor.execute("""
        ...
        # Regime forecast table (ML predictions for next month)
        cursor.execute("""
    ...
    def save_regime_labels(self, df: pd.DataFrame) -> None:
        ...
        Args:
            df: DataFrame with columns [date (index), regime, risk_on]
        """
    ...
    def load_regime_labels(self) -> pd.DataFrame:
        ...
        Returns:
            DataFrame with date index and [regime, risk_on] columns
        """
        df = pd.read_sql(
    ...
    def save_optimal_allocations(self, allocations: dict[str, dict[str, float]]) -> None:
        ...
        Args:
            allocations: Dict mapping regime -> {asset: weight}
        """
        rows = []
    ...
    def load_optimal_allocations(self) -> dict[str, dict[str, float]]:
        ...
        Returns:
            Dict mapping regime -> {asset: weight}
        """
    ...
    def save_current_weights(self, date: str, weights: pd.Series) -> None:
        ...
        Args:
            ...
            weights: Series mapping asset -> weight
        """
        # Delete existing weights for this date first
        cursor = self.conn.cursor()
    ...
    def save_backtest_results(self, metrics: dict[str, float], bench_metrics: dict[str, float]) -> None:
        ...
        Args:
            metrics: Portfolio performance dict
            bench_metrics: Benchmark performance dict
        """
        cursor = self.conn.cursor()
        cursor.execute(
            """
    ...
    def get_latest_backtest_results(self) -> dict[str, Any] | None:
        ...
        Returns:
            Dict with portfolio and benchmark metrics, or None if no results
        """
        df = pd.read_sql(
