...
def fetch_live_positions_and_nav() -> tuple[list[PositionRow], float] | None:
    ...
    try:
        ...
        if portfolio:
            ...
            logger.info(
                "Using broker portfolio with market value/price for %d positions", len(rows)
            )
            return rows, nav
...
def save_report(preview: RebalancePreview, report_dir: Path, run_id: str) -> list[Path]:
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"Target weights: {len(preview.target_weights)} entries ({num_tradable} tradable + cash).\n"
        )
        f.write(
            f"Proposed orders: {len(preview.proposed_orders)} (cash is not traded as a security).\n"
        )
        if preview.symbols_missing_price:
            f.write(
                f"Symbols missing price (excluded from orders): {preview.symbols_missing_price}\n"
            )
        f.write("\n--- Proposed orders ---\n")
        for o in preview.proposed_orders:
            f.write(
                f"  {o.side} {o.shares} {o.symbol} @ ~{o.price_used:.2f} (~${o.approximate_dollar:.2f})\n"
            )
    ...
    if use_mock_only:
        ...
        if not path or not path.exists():
            raise FileNotFoundError(
                "use_mock_only=True but mock_positions_path not provided or missing"
            )
...
def _smoke_test_checks(config: dict[str, Any]) -> None:
    ...
    if not config.get("trading_enabled", False):
        raise ValueError(
            "Smoke-test submission refused: trading_enabled is false in config. Set to true to allow."
        )
    if not config.get("paper_only", True):
        raise ValueError(
            "Smoke-test submission refused: paper_only must be true. Live trading is not supported."
        )
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Monthly rebalance: dry-run (default) or paper order submission"
    )
    ...
    p.add_argument(
        ...
        action="store_false",
        dest="dry_run",
        help="Allow submission when combined with --submit-paper-orders",
    )
    p.add_argument(
        "--submit-paper-orders",
        action="store_true",
        help="Submit paper orders (requires trading_enabled, live broker, no duplicate run)",
    )
    p.add_argument(
        "--smoke-test-symbol",
        type=str,
        default="",
        help="Smoke-test: submit one small order for this symbol only (e.g. SPY)",
    )
    p.add_argument(
        "--smoke-test-shares",
        type=int,
        default=1,
        help="Smoke-test: number of shares (1-10, default 1)",
    )
    p.add_argument(
        "--smoke-test-side", type=str, default="BUY", help="Smoke-test: BUY or SELL (default BUY)"
    )
    ...
    p.add_argument(
        ...
        type=str,
        default="",
        help="Path to mock positions JSON/CSV (fallback; not used when submitting)",
    )
    p.add_argument(
        "--use-mock-only",
        action="store_true",
        help="Skip broker; use mock positions only (incompatible with --submit-paper-orders)",
    )
    p.add_argument(
        "--prices", type=str, default="", help="Optional JSON symbol->price for share conversion"
    )
...
def _validate_arg_combinations(args, submit_paper: bool, smoke_mode: bool) -> int | None:
    ...
    if submit_paper and args.use_mock_only:
        print(
            "FAILED: --submit-paper-orders cannot be used with --use-mock-only. Submission requires live broker.",
            file=sys.stderr,
        )
        return 1
    ...
    if smoke_mode and (not submit_paper or args.dry_run):
        print(
            "FAILED: Smoke-test requires --submit-paper-orders and --no-dry-run.", file=sys.stderr
        )
        return 1
    return None
...
def _gate_submission(preview, config, rebal, report_dir, summary, safety_cfg) -> "Path | None":
    ...
    if rebal.get("duplicate_run_protection", True) and duplicate_run_refuse(marker_path):
        print(
            "FAILED: Duplicate run protection. Paper submission already performed today.",
            file=sys.stderr,
        )
...
def _execute_paper_submission(
    ...
    write_submission_marker(
        marker_path, run_id=run_id, submitted_order_count=len(results), dry_run=False
    )
...
def _do_main_flow(args, smoke_mode: bool, smoke_symbol: str, submit_paper: bool, summary):
    ...
    preview = run_dry_run(
        ...
        report_dir=report_dir,
        use_mock_only=args.use_mock_only,
    )
    _record_preview_metrics(summary, preview)
    if submit_paper:
        return (
            _execute_paper_submission(
...
def main() -> int:
    ...
    code: int | None = None
    try:
        code, preview, report_dir = _do_main_flow(
            args, smoke_mode, smoke_symbol, submit_paper, summary
        )
